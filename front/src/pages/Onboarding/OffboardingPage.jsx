...
export default function OffboardingPage({ apiUrl, token }) {
    ...
    const loadTasks = async () => {
        setLoading(true);
        try {
            const data = await api.get('/api/onboarding/me?type=offboarding');
            setTasks(data);
        } catch (e) {
            toast.error('Error al cargar checklist de salida');
        } finally {
            setLoading(false);
        }
    };
    ...
    if (loading) return <div className="p-8 text-center">Cargando tu proceso de salida...</div>;

    if (tasks.length === 0) {
        return (
            <div className="p-8 text-center">
                <h1 className="text-2xl font-bold text-slate-900">Proceso de Salida (Offboarding)</h1>
                <p className="text-slate-500 mt-2">No tienes tareas de offboarding asignadas en este momento.</p>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 max-w-4xl mx-auto">
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-slate-900">Proceso de Salida (Offboarding)</h1>
                <p className="text-slate-500 mt-2">Por favor completa estos pasos para finalizar tu proceso de salida.</p>
